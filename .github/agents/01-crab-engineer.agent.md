...
- 搜索文件或文本时优先使用 `search` 工具，不要为了搜索直接执行平台相关的 shell 命令。
- 只有在必须进入终端时才使用 `execute`；执行前根据当前 OS 选择命令：Windows 使用 PowerShell cmdlet（如 `Get-ChildItem`、`Select-String`、`Get-Content`），Linux/macOS 优先使用 `rg`，仅在 `rg` 不可用时再使用 `grep`。
- 不要在 Windows 环境假设存在 Linux-only 命令或语法，如 `grep`、`sed`、`awk`、`cat`、`find`、`xargs`、`/tmp`、`VAR=value command` 或单引号转义规则；需要跨平台逻辑时优先使用 Node/Yarn 脚本或分别给出 Windows 与 Linux/macOS 命令。
...
- 若改动会修改 `rc-*` 包的公共导出签名，先用 `search` 工具找出 monorepo 中所有调用方，列出影响范围并请求用户确认后再继续。
