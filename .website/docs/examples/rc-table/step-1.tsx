...
export default function Example() {
    return <div className={layout}>
        <strong>企业采购订单台账 · 2,000 行 × 20 列</strong>
        <p>横向滚动核对商品、金额、收款与交付日期。订单号固定，折后订单额和待收款由同一条记录计算。</p>
        <div className={frame}><AutoSizer>{({ width, height }) => width > 0 && height > 0
            ? <Table aria-label="企业采购订单台账" width={width} height={height} columns={columns} rows={rows} /> : null}</AutoSizer></div>
    </div>;
}
