...
async function click(name: string) {
    await act(async () => { await fireEvent.click(screen.getByRole("button", { name })); });
}
async function change(name: string, value: string) {
    await act(async () => { await fireEvent.change(screen.getByRole("textbox", { name }), { target: { value } }); });
}

describe("设计样板资料编辑", () => {
    it("取消有效草稿不会提交，确认放弃后恢复已保存值", async () => {
        const save = mock.fn(async (profile: Profile) => profile);
        await act(async () => { await render(<ProfileExample theme="light" density="standard" save={save} />); });
        await click("编辑成员资料");
        ...
        await click("放弃修改");
        await click("编辑成员资料");
        expect(screen.getByRole("textbox", { name: "姓名（必填）" })).toHaveProperty("value", "林晓");
    });
    it("校验定位字段，取消确认保留草稿，放弃修改恢复原值", async () => {
        await act(async () => { await render(<ProfileExample theme="dark" density="compact" />); });
        await click("编辑成员资料");
        await change("姓名（必填）", "");
        await click("保存修改");
        ...
        await click("取消编辑");
        await click("继续编辑");
        expect(screen.getByRole("textbox", { name: "姓名（必填）" })).toHaveProperty("value", "");
        await click("取消编辑");
        await click("放弃修改");
        await click("编辑成员资料");
        expect(screen.getByRole("textbox", { name: "姓名（必填）" })).toHaveProperty("value", "林晓");
    ...
    it("提交期间阻止重复保存，成功后回写摘要", async () => {
        ...
        await act(async () => { complete?.({ name: "王宁", email: "lin@example.com", team: "产品设计" }); });
    ...
    it("保存失败保留草稿并允许重试", async () => {
        ...
        expect(screen.getByRole("textbox", { name: "姓名（必填）" })).toHaveProperty("value", "王宁");
    });
});
