...
import { Alert, Button, Dialog, Drawer, Empty, LineEdit, Tag } from "@crab-dev/wake/internal/components-runtime";
...
export function CodeBlock({ language, code, title, children }: { language: string; code: string; title?: string; children: React.ReactNode }) {
  ...
  return <figure className="code-block" data-language={language} data-line-numbers={lineCount > 1 ? "true" : "false"}>
    <figcaption className="code-toolbar">
      ...
      <Button appearance="text" type="button" className={copyStatus === "copied" ? "is-copied" : copyStatus === "error" ? "is-error" : ""} onClick={copy} aria-label={text("Copy code", "复制代码")} aria-live="polite">{copyLabel}</Button>
    </figcaption>
  ...
  return <div className="demo-card" ref={ref}>
    ...
    <div className="demo-toolbar">
      <div className="demo-device-tools" role="group" aria-label={text("Preview device", "预览设备")}>
        {viewportOptions.map((option) => <Button appearance="text" className="docs-demo-tool" aria-pressed={viewport === option.id} aria-label={option.label + " · " + option.size} data-tooltip={option.label + " · " + option.size} type="button" key={option.id} onClick={() => setViewport(option.id)}><i className={"viewport-icon viewport-icon-" + option.id} aria-hidden="true" /></Button>)}
      </div>
      <div className="demo-toolbar-actions">
        <Button appearance="text" className="docs-demo-tool" id={codeToggleId} type="button" aria-label={codeOpen ? text("Collapse source", "收起源码") : text("Expand source", "展开源码")} aria-expanded={codeOpen} aria-controls={codePanelId} data-tooltip={codeOpen ? text("Collapse source", "收起源码") : text("Expand source", "展开源码")} onClick={() => setCodeOpen((open) => !open)}><i className="demo-tool-icon demo-tool-icon-code" aria-hidden="true" /></Button>
        {codeOpen && <Button appearance="text" className="docs-demo-tool" type="button" onClick={copy} aria-label={text("Copy demo source", "复制演示源码")} data-tooltip={copied ? text("Copied", "已复制") : text("Copy", "复制")}><i className="demo-tool-icon demo-tool-icon-copy" aria-hidden="true" /></Button>}
        <Button appearance="text" className="docs-demo-tool" type="button" aria-label={text("Open playground", "全屏预览")} data-tooltip={text("Open playground", "全屏预览")} aria-haspopup="dialog" onClick={() => setFullscreen(true)}><i className="demo-tool-icon demo-tool-icon-fullscreen" aria-hidden="true" /></Button>
      </div>
    </div>
    ...
    <Dialog className="docs-playground-dialog" open={fullscreen} onOpenChange={setFullscreen}
      title={meta.title || demo.title} maskClosable
      i18n={{ cancelText: text("Close", "关闭"), confirmText: text("Close", "关闭") }}>
      {fullscreen && <div className="playground-body">{preview}<DemoSource source={source} highlighted={highlightedSource} language={sourceLanguage} /></div>}
    </Dialog>
  </div>;
}
...
export function API({ source, symbol, __wakePage }: { source: string; symbol: string; component?: string; __wakePage: string }) {
  ...
  return <section className="api-section" aria-labelledby={"api-" + symbol}>
    <div className="api-heading">
      ...
      <div className="api-filter"><LineEdit aria-label={text("Filter properties", "筛选属性")} aria-describedby={statusId} placeholder={text("Filter by name or type…", "按名称或类型筛选…")} value={filter} onChange={(event) => setFilter(event.target.value)} /></div>
    </div>
    ...
    </tbody></table></div> : <div className="api-empty"><strong>{text("No matching properties", "没有匹配的属性")}</strong><span>{text("Try another name or type keyword.", "请尝试其他属性名或类型关键词。")}</span><Button appearance="text" type="button" onClick={() => setFilter("")}>{text("Clear filter", "清除筛选")}</Button></div>}
...
function Search({ open, close, go }: { open: boolean; close: () => void; go: (slug: string) => void }) {
  ...
  const choose = (slug: string) => { close(); go(slug); };
  ...
  return <Dialog id="wake-search-dialog" className="docs-search-dialog"
    open={open} onOpenChange={(next) => { if (!next) close(); }}
    title={text("Search documentation", "搜索文档")} maskClosable
    i18n={{ cancelText: text("Close", "关闭"), confirmText: text("Open result", "打开结果") }}
    onConfirm={() => { if (!results[active]) return false; choose(results[active].slug); }}>
    <LineEdit autoFocus ref={input} value={query} onChange={(event) => { setQuery(event.target.value); setActive(0); }} onKeyDown={onKeyDown} role="combobox" aria-label={text("Search documentation", "搜索文档")} aria-autocomplete="list" aria-expanded={open} aria-controls="wake-search-results" aria-activedescendant={results[active] ? "wake-search-result-" + active : undefined} placeholder={text("Search pages, headings, commands, and props…", "搜索页面、章节、命令和属性…")} />
    <div className="search-results" id="wake-search-results" role="listbox" aria-label={text("Search results", "搜索结果")}>
      {results.map((item: any, index: number) => <Button appearance="text" shouldFitContainer
        id={"wake-search-result-" + index} className="docs-search-result"
        role="option" aria-selected={index === active} isSelected={index === active}
        tabIndex={-1} key={item.slug + index}
        onMouseMove={() => setActive(index)} onMouseDown={(event) => event.preventDefault()}
        onClick={() => choose(item.slug)}>
        <span><strong>{item.title}</strong><small>{item.detail}</small></span><Tag>{item.kind}</Tag>
      </Button>)}
      {!results.length && <Empty title={text("No results", "没有匹配结果")} description={query} />}
    </div>
  </Dialog>;
}
...
function ThemeButton({ theme, setTheme }: { theme: Theme; setTheme: (theme: Theme) => void }) {
  ...
  return <Button appearance="text" type="button" className="docs-icon-control" onClick={() => setTheme(next[theme])} aria-label={label} title={label}>{icon}</Button>;
}
...
function Sidebar({ current, close }: { current: string; close?: () => void }) {
  ...
  const link = (page: PageRecord, nested = false) => {
    ...
    return <Button appearance="text" shouldFitContainer isSelected={active}
      key={page.slug}
      className={"docs-nav-link" + (nested ? " docs-nav-nested" : "")}
    ...
    {groups.map((group) => <div className="nav-group" key={group.id}>
      ...
      {group.sections.map((section) => {
        ...
        return <div className="nav-section" key={key}>
          <Button appearance="text" type="button" className="docs-nav-toggle" aria-expanded={open} aria-controls={controls} onClick={() => setExpandedByUser((current) => {
...
function ReadingProgress() {
  ...
  return <>
    ...
    {showBackToTop && <Button appearance="text" type="button" className="docs-back-to-top" aria-label={text("Back to top", "返回顶部")} onClick={() => window.scrollTo({ top: 0, behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" })}>↑</Button>}
  </>;
}
...
class PageErrorBoundary extends React.Component<{ children: React.ReactNode }, { error: string }> {
  ...
  render() {
    return this.state.error ? <div className="frame-error" role="alert">
      ...
      <Button appearance="text" type="button" onClick={() => location.reload()}>{text("Retry", "重试")}</Button>
...
function DemoFrame({ id, resolved }: { id: string; resolved: ResolvedTheme }) {
  ...
  return <DemoErrorBoundary resetKey={argsRevision} onError={reportError}>
    <div className="demo-frame-root" data-wake-demo><Preview><Component {...args} /></Preview></div>
  </DemoErrorBoundary>;
}
...
export function App() {
  ...
  return <div className="docs-shell">
    ...
    <header className="topbar">
      <Button appearance="text" type="button" className="mobile-menu docs-icon-control" aria-haspopup="dialog" aria-expanded={drawer} aria-controls="wake-docs-drawer" onClick={() => setDrawer(true)} aria-label={text("Open navigation", "打开导航")}>☰</Button>
      <Logo />
      <div className="topbar-actions">
        <Button appearance="text" type="button" className="docs-search-trigger" aria-haspopup="dialog" aria-controls="wake-search-dialog" aria-keyshortcuts="Control+K Meta+K" aria-expanded={search} onClick={() => setSearch(true)}><span>⌕ {text("Search", "搜索")}</span><kbd>{searchShortcut}</kbd></Button>
    ...
    <Drawer id="wake-docs-drawer" open={drawer} onOpenChange={setDrawer}
      placement="left" size="small" title={text("Documentation navigation", "文档导航")}
      closeLabel={text("Close navigation", "关闭导航")}>
      <Sidebar current={page?.slug || ""} close={() => setDrawer(false)} />
    </Drawer>
