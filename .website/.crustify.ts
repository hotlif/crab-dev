...
const TOOLBOX_DIR = join(REPO_ROOT, "toolbox");
...
interface ToolEntry {
    ...
    role: string;
    stage: string;
    /** 流水线上的一句话说明; 比 package.json 的 description 更齐整, 专供文档站。 */
    summary: string;
    /** 这一站的产出。 */
    output: string;
    description: string;
...
/**
 * 扫描 toolbox/ 下的内部工具包。与组件不同, 工具没有 demo / docgen, 文档即 README。
 */
const scanToolbox = (): ToolEntry[] => {
    if (!existsSync(TOOLBOX_DIR)) return [];

    const entries: ToolEntry[] = [];
    const dirs = readdirSync(TOOLBOX_DIR).filter(name =>
        statSync(join(TOOLBOX_DIR, name)).isDirectory(),
    );

    for (const slug of dirs) {
        const dir = join(TOOLBOX_DIR, slug);
        const pkgPath = join(dir, "package.json");
        ...
        entries.push({
            ...
            pkg: pkgJson.name ?? `@crab-dev/${slug}`,
            version: pkgJson.version ?? "0.0.0",
            title: pkgJson.websiteConfig?.title ?? slug,
            role: pkgJson.websiteConfig?.role ?? "",
            stage: pkgJson.websiteConfig?.stage ?? "",
            summary: pkgJson.websiteConfig?.summary ?? pkgJson.description ?? "",
            output: pkgJson.websiteConfig?.output ?? "",
            description: pkgJson.description ?? "",
            order: pkgJson.websiteConfig?.order ?? 99,
...
const escapeTpl = (raw: string): string =>
...
const writeToolboxManifest = (entries: ToolEntry[]): void => {
    if (!existsSync(GENERATED_DIR)) mkdirSync(GENERATED_DIR, { recursive: true });

    const lines: string[] = [];
    lines.push("// Auto-generated by .crustify.ts. Do not edit.");
    lines.push("export interface ToolManifestItem {");
    lines.push("    slug: string;");
    lines.push("    pkg: string;");
    lines.push("    version: string;");
    lines.push("    title: string;");
    ...
    lines.push("}");
    lines.push("");
    lines.push("const toolboxManifest: ToolManifestItem[] = [");
    for (const e of entries) {
        lines.push("    {");
        lines.push(`        slug: ${JSON.stringify(e.slug)},`);
        lines.push(`        pkg: ${JSON.stringify(e.pkg)},`);
        ...
        lines.push(`        stage: ${JSON.stringify(e.stage)},`);
    ...
    lines.push("];");
    lines.push("");
    lines.push("export default toolboxManifest;");
    lines.push("");

    writeFileSync(join(GENERATED_DIR, "toolboxManifest.ts"), lines.join("\n"), "utf-8");
};
...
class WebsiteMod implements Modification {
    constructor() {
        ...
        writeToolboxManifest(scanToolbox());
    }
