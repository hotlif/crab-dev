---
name: workflow
description: "crab-dev monorepo 组件开发的端到端实战流程与可复制模板。**触发场景**：新建或脚手架化 rc-* 组件；编辑 token.toml 并运行 generate:token；配置或排错 Packify(Rollup) / Crustify(Webpack) / Lignify / Turbo 任务图；编写 Jest 30 + RTL 测试（ESM + experimental-vm-modules）；处理 ESM 相对导入 .js 扩展名、bare specifier、moduleResolution: bundler；新增 *.demo.tsx / *.view.tsx / *.mdx；定义 index.ts 导出形态；落实验收命令矩阵。**不要用于**：纯样式质感打磨（→ /style）；workspace 包链接 / TS2307 / qualified path resolution failed（→ /link-packages）；非 crab-dev 仓库的通用建议。"
---

# 组件开发工作流（Component Workflow）

> 本命令聚焦可操作流程与实战模板。通用规范（代码风格、提交规范、验收清单等）见 `CLAUDE.md`，此处不重复。
...
> **样式注意**：编写组件源码中的 Linaria 样式时，**应**同步参考 `/style` 命令以确保状态覆盖与令牌合规。
...
优先使用 Grep 工具，在 `components/rc-{name}/package.json` 中搜索 `rc-component-preview|rc-menu|rc-masonry`。

若必须走终端：

```powershell
# Windows PowerShell
Select-String -Path "components\rc-{name}\package.json" -Pattern "rc-component-preview|rc-menu|rc-masonry"
```

```bash
# Linux / macOS
rg "rc-component-preview|rc-menu|rc-masonry" components/rc-{name}/package.json
```
...
> `cd ... && yarn ...` 中 `&&` 在 PowerShell 7+ 与 Bash 中均可用。Windows 路径分隔符用 `\` 或带引号的 `/`。
...
| `Qualified path resolution failed` | 先构建目标包：`yarn build:library` |
...
详见 [.claude/commands/references/build-toolchain.md](.claude/commands/references/build-toolchain.md)。