...
export const rootStyle = css`
    ...
    border-radius: ${token.root.radius}; background: ${token.root.background}; color: ${token.root.color};
...
export const headingStyle = css`margin: 0; font-size: ${token.panel['heading-size']}; font-weight: ${token.panel['heading-weight']};`;
...
export const progressSlotStyle = css`display: flex; align-items: center; width: ${token.toolbar['icon-size']}; height: ${token.toolbar['icon-size']}; flex-shrink: 0;`;
...
`;
export const toolDividerStyle = css`height: ${token.toolbar['icon-size']}; width: ${token.root['border-width']}; background: ${token.root['border-color']}; margin-inline: ${token.toolbar.gap};`;
...
export const statusStyle = css`display: flex; align-items: center; flex-wrap: wrap; gap: ${token.toolbar.gap}; padding: ${token.status['padding-block']} ${token.panel.padding}; color: ${token.status.color}; font-size: ${token.status.size};`;
export const hintStyle = css`margin: 0; color: ${token.status.color}; font-size: ${token.status.size}; overflow-wrap: anywhere;`;
export const errorStyle = css`padding: ${token.toolbar.gap} ${token.panel.padding}; color: ${token.error.color}; background: ${token.error.background};`;
export const thumbStyle = css`
    ...
    &[data-drop]::after { content: ''; position: absolute; z-index: 1; inset-inline: 0; height: ${token.thumbnail['drop-width']}; background: ${token.thumbnail['drop-color']}; pointer-events: none; }
...
export const thumbButtonStyle = css`
    ...
    &[data-dragging='true'] { cursor: grabbing; background-color: ${token.thumbnail['drag-background']}; }
`;
export const thumbContentStyle = css`display: flex; flex-direction: column; align-items: center; font-size: ${token.status.size};`;
...
export const objectTreeStyle = css`
    ...
    & svg { width: ${token.objects['icon-size']}; height: ${token.objects['icon-size']}; }
`;
export const iconStyle = css`width: ${token.toolbar['icon-size']}; height: ${token.toolbar['icon-size']}; display: block; flex-shrink: 0; & > svg { width: 100%; height: 100%; }`;
export const toolbarButtonStyle = css`
    flex-shrink: 0;
    ${buttonVars['icon.width']}: ${token.toolbar['icon-size']};
    ${buttonVars['size.middle.icon.width']}: ${token.toolbar['icon-size']};
    ${buttonVars['text.color']}: ${token.toolbar['icon-color']};
    ${buttonVars['text.background-color-hover']}: ${token.toolbar['icon-hover']};
    ${buttonVars['text.background-color-focus']}: ${token.toolbar['icon-focus']};
    ${buttonVars['text.background-color-active']}: ${token.toolbar['icon-pressed']};
    &::after {
        content: ''; position: absolute; inset: 50% auto auto 50%; transform: translate(-50%, -50%);
        width: max(100%, ${token.toolbar['target-size']}); height: max(100%, ${token.toolbar['target-size']});
    }
`;
