...
const TYPE_LOADERS: DataTypeLoader[] = [
    ...
    },
    {
        name: "percent",
        render: ({ row, column }) => {
            const field = String(column.name).replace(/^\$\./, "") as keyof MetricRow["dataRef"];
            const n = Number((row as MetricRow).dataRef[field] ?? 0);
    ...
    },
    {
        ...
        render: ({ row, column }) => {
            const field = String(column.name).replace(/^\$\./, "") as keyof MetricRow["dataRef"];
            const s = String((row as MetricRow).dataRef[field] ?? "");
