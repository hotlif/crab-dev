import { describe, expect, it, beforeEach, mock } from "@crab-dev/wake/test";
...
describe("sortRules (tested via TreeDataUtil.moveNodeOnDrag)", () => {
    it("sorts by priority ascending", () => {
        ...
        const dragNode = createNode("drag", { parent: parentNode, priority: 10 });
        let captured: Node[] = [];
        const onTreeNodeChange = mock.fn((updater: SetStateAction<Node[]>) => {
            const currentTreeData = [parentNode, nodeA, nodeB, dragNode];
            captured = typeof updater === "function" ? updater(currentTreeData) : updater;
        });
        const util = new TreeDataUtil({
            treeData: [parentNode, nodeA, nodeB, dragNode],
            onTreeNodeChange,
        });
        // Move drag above nodeB (priority 1, first in sort order)
        util.moveNodeOnDrag("drag", "b", OverStateEnum.UPWARD);
        expect(captured.find(node => node.id === "drag")?.priority).toBe(0.5);
    });
});
describe("TreeDataUtil", () => {
    let treeData: Node[];
    let onTreeNodeChange: MockFunction;
    ...
    beforeEach(() => {
        ...
        onTreeNodeChange = mock.fn((updater: SetStateAction<Node[]>) => {
    ...
    describe("reloadChildrenByParent", () => {
        it("reloads root children when parent is null", async () => {
            const loadData = mock.fn(() => Promise.resolve([createNode("new-1"), createNode("new-2")]));
        ...
        it("reloads children for specific parent", async () => {
            ...
            const loadData = mock.fn(() => Promise.resolve([newChild]));
            await util.reloadChildrenByParent({
                parent,
                loadData,
                expandedKeys: [],
...
describe("loadDataFunc", () => {
    it("loads root data and assigns priority", async () => {
        ...
        const loadData = mock.fn(() => Promise.resolve(nodes));
    ...
    it("recursively loads expanded children", async () => {
        ...
        const loadData = mock.fn<(parentNode: Node | null) => Promise<Node[]>>()
            .resolveOnce(rootNodes)
            .resolveOnce(childNodes);
        const result = await loadDataFunc({
            parentNode: null,
            loadData,
            expandedKeys: ["root"],
    ...
    it("returns empty array when loadData returns undefined", async () => {
        const loadData = mock.fn(() => Promise.resolve(undefined as never));
        const result = await loadDataFunc({
            parentNode: null,
            loadData,
            expandedKeys: [],
    ...
    it("sets parent on loaded child nodes", async () => {
        ...
        const loadData = mock.fn(() => Promise.resolve(children));
        const result = await loadDataFunc({
            parentNode: parent,
            loadData,
            expandedKeys: [],
