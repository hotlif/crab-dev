import { act, describe, it, expect, mock, render, fireEvent, screen } from "@crab-dev/wake/test/react";
...
describe("Virtual", () => {
    ...
    describe("scrollbar interaction", () => {
        it("should update scroll position when y-scrollbar thumb is dragged", async () => {
            ...
            const callCountBefore = props.renderRows.calls.calls.length;
            await act(async () => {
                await fireEvent(yThumb, new PointerEvent("pointerdown", { bubbles: true, button: 0, clientY: 5, pointerId: 1 }));
            });
            await act(async () => {
                await fireEvent(yThumb, new PointerEvent("pointermove", { bubbles: true, clientY: 30, pointerId: 1 }));
            });
            expect(props.renderRows.calls.calls.length).toBeGreaterThan(callCountBefore);
            await act(async () => {
                await fireEvent(yThumb, new PointerEvent("pointerup", { bubbles: true, pointerId: 1 }));
            });
        });
        it("should update scroll position when x-scrollbar thumb is dragged", async () => {
            ...
            const callCountBefore = props.renderRows.calls.calls.length;
            await act(async () => {
                await fireEvent(xThumb, new PointerEvent("pointerdown", { bubbles: true, button: 0, clientX: 5, pointerId: 1 }));
            });
            await act(async () => {
                await fireEvent(xThumb, new PointerEvent("pointermove", { bubbles: true, clientX: 30, pointerId: 1 }));
            });
            expect(props.renderRows.calls.calls.length).toBeGreaterThan(callCountBefore);
            await act(async () => {
                await fireEvent(xThumb, new PointerEvent("pointerup", { bubbles: true, pointerId: 1 }));
            });
        });
    });
    describe("edge cases", () => {
