...
import {
    type Pt, type Rect, type Side, type EdgeAnchor,
    anchorPoint, nearestSideAnchor, dominantSide, isHorizontalSide,
    segmentIntersectsRect, simplifyOrthogonal,
    routeOrthogonal, connectThroughWaypoints, withTerminalStubs,
    routeEdge, routeAvoidingObstacles, assignPorts,
...
const entersAlong = (pts: Pt[], side: Side) =>
    legHorizontal(pts[pts.length - 2], pts[pts.length - 1]) === isHorizontalSide(side);
function occludes(pts: Pt[], rects: Rect[]): boolean {
    for (let i = 0; i < pts.length - 1; i++) {
        for (const r of rects) if (segmentIntersectsRect(pts[i], pts[i + 1], r)) return true;
    }
    return false;
}
