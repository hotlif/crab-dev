...
describe('zIndexPath - Stacking Context 语义', () => {
    ...
    it('Group 内叶子的 zIndexPath = [groupZIndex, leafZIndex]', () => {
        const ctx = makeCtxMock();
        render(
            <TestWrapper ctx={ctx}>
                <Group zIndex={0}>
    ...
    it('Group(zIndex=1) 内的元素路径 > Group(zIndex=0) 内的高 zIndex 元素', () => {
        const ctx = makeCtxMock();
        render(
            <TestWrapper ctx={ctx}>
                <Group zIndex={0}>
        ...
        const calls = (ctx.register as jest.Mock).mock.calls as Array<[{ zIndexPath: number[] }]>;
