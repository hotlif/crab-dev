...
describe('Group', () => {
    it('平移：子组件 worldMatrix 将 (0,0) 映射到 (x, y)', () => {
        ...
        const cmd = (ctx.register as jest.Mock).mock.calls[0]?.[0] as { worldMatrix: Float32Array };
        const [ox, oy] = applyMat3(cmd.worldMatrix, 0, 0);
        expect(ox).toBeCloseTo(30);
        expect(oy).toBeCloseTo(50);
    });

    it('缩放：子组件 worldMatrix 将 (1,1) 映射到 (scaleX, scaleY)', () => {
        const ctx = makeCtxMock();
        render(
            <TestWrapper ctx={ctx}>
    ...
    it('嵌套 Group：矩阵叠加，平移 (10,0) 内再平移 (5,0)，合计 (15,0)', () => {
        const ctx = makeCtxMock();
        render(
            <TestWrapper ctx={ctx}>
                <Group x={10} y={0}>
    ...
    it('zIndexPath：Group 将自身 zIndex 追加到路径，子 Rect 再追加自身 zIndex', () => {
        const ctx = makeCtxMock();
        render(
            <TestWrapper ctx={ctx}>
                <Group zIndex={10}>
    ...
    it('无 children 时不报错', () => {
        const ctx = makeCtxMock();
        expect(() => render(<TestWrapper ctx={ctx}><Group /></TestWrapper>)).not.toThrow();
    });

    it('不注册 DrawCommand，只传递 Context', () => {
        const ctx = makeCtxMock();
        render(
            <TestWrapper ctx={ctx}>
                <Group x={5} y={5} />
