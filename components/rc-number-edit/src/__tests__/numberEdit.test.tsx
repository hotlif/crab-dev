...
jest.mock("@crab-dev/rc-line-edit", () => ({
    __esModule: true,
    default: ({
        ref,
        containerRef,
        ...
        <div ref={containerRef} data-testid="line-edit">
            {prefix}
            <input
                ref={ref}
                value={value}
                ...
                {...rest}
            />
            {allowClear && typeof value === "string" && value.length > 0 && !disabled && !readOnly && (
                <button type="button" aria-label="清除" onClick={onClear}>x</button>
            )}
            {suffix}
        </div>
    ),
...
describe("NumberEdit", () => {
    describe("基础渲染与无障碍", () => {
        it("以 spinbutton 语义渲染并带 aria 范围", async () => {
            await act(async () => { render(<NumberEdit value={5} min={0} max={10} />); });
        ...
        it("默认显示步进按钮，controls={false} 时隐藏", async () => {
            const { rerender } = render(<NumberEdit value={1} />);
            await act(async () => {});
            expect(stepUp()).toBeTruthy();
            expect(stepDown()).toBeTruthy();
            await act(async () => { rerender(<NumberEdit value={1} controls={false} />); });
    ...
    describe("失焦态显示", () => {
        it("普通十进制原样显示", async () => {
            await act(async () => { render(<NumberEdit value={1234.5} />); });
            expect(getInput().value).toBe("1234.5");
        });

        it("开启千分位后分组显示", async () => {
            await act(async () => { render(<NumberEdit value={1234567} thousandSeparator />); });
            expect(getInput().value).toBe("1,234,567");
        });

        it("超阈值数值以上标科学计数法覆盖显示", async () => {
            await act(async () => { render(<NumberEdit value={1.23e21} />); });
    ...
    describe("聚焦编辑态", () => {
        ...
        it("失焦时将超界值钳制到范围（非受控）", async () => {
            ...
            await act(async () => {
                render(<NumberEdit defaultValue={0} max={100} onChange={onChange} />);
            });
            const input = getInput();
            act(() => { fireEvent.focus(input); });
    ...
    describe("步进", () => {
        it("点击增加按钮触发 onChange(+step)", async () => {
            const onChange = jest.fn();
            await act(async () => { render(<NumberEdit value={5} onChange={onChange} />); });
            act(() => {
                fireEvent.pointerDown(stepUp());
                fireEvent.pointerUp(stepUp());
            });
        ...
        it("键盘 ArrowUp 步进一个 step", async () => {
            const onChange = jest.fn();
            await act(async () => { render(<NumberEdit value={5} onChange={onChange} />); });
            act(() => { fireEvent.keyDown(getInput(), { key: "ArrowUp" }); });
            expect(onChange).toHaveBeenCalledWith(6);
        });
        ...
        it("Shift+ArrowUp 走大步长（默认 step*10）", async () => {
            const onChange = jest.fn();
            await act(async () => { render(<NumberEdit value={5} onChange={onChange} />); });
            act(() => { fireEvent.keyDown(getInput(), { key: "ArrowUp", shiftKey: true }); });
            expect(onChange).toHaveBeenCalledWith(15);
        });

        it("到达上界时禁用增加按钮", async () => {
            await act(async () => { render(<NumberEdit value={10} max={10} />); });
        ...
        it("步进消除浮点噪声", async () => {
            const onChange = jest.fn();
            await act(async () => { render(<NumberEdit value={0.1} step={0.2} onChange={onChange} />); });
            act(() => { fireEvent.keyDown(getInput(), { key: "ArrowUp" }); });
    ...
    describe("清除与禁用", () => {
        it("allowClear 清除后 onChange(null)", async () => {
            ...
            await act(async () => {
                render(<NumberEdit value={5} allowClear onChange={onChange} onClear={onClear} />);
            });
        ...
        it("disabled 时 input 与步进按钮均禁用", async () => {
            await act(async () => { render(<NumberEdit value={5} disabled />); });
