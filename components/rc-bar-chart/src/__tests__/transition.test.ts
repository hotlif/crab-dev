import { describe, it, expect, beforeEach, afterEach, mock, renderHook, act } from "@crab-dev/wake/test/react";
import { createElement, StrictMode } from 'react';
import { easeOutCubic, barProgress, sampleBars, staggerStep, totalSpan, barKey, useBarTransition, type BarGeom, type BarTransitionOptions, } from '../hooks/useBarTransition.js';
...
describe('useBarTransition（resize 即时同步与 ready 门控）', () => {
    ...
    beforeEach(() => {
        ...
        mock.spyOn(globalThis, 'cancelAnimationFrame').implement(id => {
    ...
    it('入场首帧为基线零厚，动画帧沿值轴生长（x 不滑移）', async () => {
        const { result } = await renderHook(() => useBarTransition(barsAt(600), opts(600)));
    ...
    it('宽度变化（容器 resize）：立即同步新布局终态，不补间不错峰', async () => {
        const { result, rerender } = await renderHook(({ w }) => useBarTransition(barsAt(w), opts(w)), { initialProps: { w: 600 } });
        await rerender({ w: 900 });
    ...
    it('数据变化（宽度不变）：从当前几何补间，柱不瞬移到终态', async () => {
        const { result, rerender } = await renderHook(({ bars }) => useBarTransition(bars, opts(600)), { initialProps: { bars: [bar({ y: 40, height: 160 })] } });
        await rerender({ bars: [bar({ y: 20, height: 180 })] });
        // 补间分支不同步落终态（首帧尚未执行），区别于 resize 的立即同步
        expect(result.current[0].height).not.toBe(180);
    });
    it('ready=false 时入场待命；ready=true 后以真实布局从基线生长', async () => {
        const { result, rerender } = await renderHook(({ w, ready }) => useBarTransition(barsAt(w), opts(w, { ready })), { initialProps: { w: 600, ready: false } });
        // 待命：不启动动画
        expect(rafCallbacks.size).toBe(0);
        await rerender({ w: 900, ready: true });
        await act(() => runFrame());
    ...
    it('staggered 缺省（分组）：同一时刻靠后类目生长落后（自左向右涌现）', async () => {
        const nowSpy = mock.spyOn(performance, 'now').return(1000);
        const { result } = await renderHook(() => useBarTransition(spreadBars, opts(600, { categoryCount: 4 })));
        nowSpy.return(1300); // elapsed=300ms，类目 3 错峰 150ms 后仅走了 150ms
        await act(() => runFrame());
        expect(result.current[1].height).toBeLessThan(result.current[0].height);
    });
    it('staggered=false（堆叠）：全类目同帧同步生长，无错峰', async () => {
        const nowSpy = mock.spyOn(performance, 'now').return(1000);
        const { result } = await renderHook(() => useBarTransition(spreadBars, opts(600, { categoryCount: 4, staggered: false })));
        nowSpy.return(1300);
        await act(() => runFrame());
        expect(result.current[0].height).toBeGreaterThan(0);
        expect(result.current[1].height).toBeCloseTo(result.current[0].height);
    });
    it('StrictMode 双挂载：重挂载视作全新入场，动画正常启动而非停留基线空白', async () => {
        const nowSpy = mock.spyOn(performance, 'now').return(1000);
        const { result } = await renderHook(() => useBarTransition(barsAt(600), opts(600)), {
