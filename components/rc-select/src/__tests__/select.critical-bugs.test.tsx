...
// 并调用它,把视口外的高亮行滚进来。
mock.module('@crab-dev/rc-virtual', async () => {
    const mockReact = await mock.actual<typeof import("react")>("react");
    function MockVirtual({ renderRows, gridTemplateRows, viewportHeight, gridRef, }: {
        renderRows: (rowRange: [
            number,
            number
        ], columnRange: [
        ...
        mockReact.useImperativeHandle(gridRef as never, () => ({
            scrollToCell: ({ rowIndex }: {
                ...
                if (rowIndex < start)
                    ...
                    setStart(rowIndex - visibleCount + 1);
            },
            getScrollCellPosition: () => ({ rowIndex: start, columnIndex: 0 }),
        }));
        return (<div data-testid="vlist" data-visible={`${start}-${end}`}>
...
describe('rc-select 严重问题回归', () => {
    ...
    it('#1: 键盘高亮到视口外的选项时,该项被滚入视口、出现在 DOM', async () => {
        const options = Array.from({ length: 20 }, (_, i) => ({
            label: `Option ${i + 1}`,
            value: `opt-${i}`,
        }));
        await render(<Select aria-label="s1" options={options}/>);
    ...
    // ── #2:过滤后重置高亮 ──────────────────────────────────────────────────────
    it('#2: 搜索过滤出唯一项后,直接按 Enter 即可选中该项', async () => {
        const onChange = mock.fn();
        await render(<Select aria-label="s2" searchable onChange={onChange} options={[
        ...
        await fireEvent.keyDown(combobox, { key: 'ArrowDown' }); // → 0 Alpha
        await fireEvent.keyDown(combobox, { key: 'ArrowDown' }); // → 1 Beta
        await fireEvent.keyDown(combobox, { key: 'ArrowDown' }); // → 2 Gamma
        // 输入 "beta" 过滤 → 结果只剩 Beta;高亮被重置到该项(索引 0)
        const input = screen.getByRole('textbox');
        await changeInputValue(input as HTMLInputElement, 'beta');
    ...
    // ── #3:分组 key 不碰撞 ─────────────────────────────────────────────────────
    it('#3: 分组标题为 ReactNode 时不产生重复 key 警告', async () => {
        const errorSpy = mock.spyOn(console, 'error').implement(() => { });
        await render(<Select aria-label="s3" options={[
            { label: <span>Fruits</span>, options: [{ label: 'Apple', value: 'apple' }] },
            { label: <span>Vegetables</span>, options: [{ label: 'Carrot', value: 'carrot' }] },
        ]}/>);
        await fireEvent.click(screen.getByRole('combobox', { name: 's3' }));
        const messages = errorSpy.calls.calls.map((call) => String(call[0]));
        errorSpy.restore();
    ...
    // 让用户自己往下翻找"选的到底是哪个"。
    it('#4: 单选已有值时,打开下拉应把选中项滚入视口', async () => {
        const options = Array.from({ length: 20 }, (_, i) => ({
            label: `Option ${i + 1}`,
            value: `opt-${i}`,
        }));
        await render(<Select aria-label="s4" defaultValue="opt-15" options={options}/>);
